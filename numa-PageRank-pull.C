...
void *PageRankSubWorker(void *arg) {
  ...
  while (1) {
    ...
    edgeMap(GA, Frontier, PR_F<vertex>(p_curr, p_next, GA.V, rangeLow, rangeHi),
            output, 0, DENSE_PARALLEL, false, true, subworker);
...
void *PageRankThread(void *arg) {
  ...
  subPartitionByDegree(GA, CORES_PER_NODE, sizeOfShards, sizeof(double),
                       rangeLow, rangeHi);
