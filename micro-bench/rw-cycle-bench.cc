...
void *sayHello(void *arg_ptr) {
  ...
  numa_bind(cpumask);
  if (arg->my_tid != 0) buf = (int *)numa_alloc_local(REGION_SIZE);
  srand(time(NULL));
  ...
  for (i = 0; i < (REGION_SIZE / sizeof(int)); i++) {
    ptr[i] = rand() % numOfInt;
    if (arg->my_tid != 0) buf[i] = rand() % numOfInt;
  }
  *myPos = (char *)ptr;
  ...
  if (arg->my_tid == 0) {
    ...
    for (i = 0; i < (REGION_SIZE / sizeof(int)); i++) {
      ...
      if (ptr[idx] < 0) ptr[idx] = 0;
    }
    gettimeofday(&end, &tz);
    int numOfAccess = REGION_SIZE / sizeof(int);
    printf("%d: cycle total %d for %d mem access, average: %f\n", arg->my_tid,
           cycleSum, numOfAccess, (float)cycleSum / (float)numOfAccess);
    // printf("%d\n", bufSum);
  } else {
    uint64_t cycleSum = 0;
    gettimeofday(&start, &tz);
...
int main(int argc, char *argv[]) {
  ...
  randomAccess = false;
  if (argc > 4)
    if ((std::string)argv[4] == (std::string) "-random") randomAccess = true;
