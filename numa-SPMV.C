...
struct SPMV_F {
  ...
  intT rangeLow;
  intT rangeHi;
  SPMV_F(double *_p_curr, double *_p_next, vertex *_V, intT _rangeLow,
         intT _rangeHi)
      : p_curr(_p_curr),
        p_next(_p_next),
        V(_V),
  ...
  inline bool update(
      intT s, intT d,
      intE edgeLen) {  // update function applies PageRank equation
...
struct SPMV_Vertex_Reset {
  double *p_curr;
  SPMV_Vertex_Reset(double *_p_curr) : p_curr(_p_curr) {}
...
void *SPMVSubWorker(void *arg) {
  ...
  while (1) {
    ...
    pthread_barrier_wait(&global_barr);
    edgeMapDenseForward(GA, All,
                        SPMV_F<vertex>(p_curr, p_next, GA.V, rangeLow, rangeHi),
                        output, true, start, end);
    // edgeMapDenseForwardDynamic(GA, All, SPMV_F<vertex>(p_curr, p_next, GA.V,
    // rangeLow, rangeHi), output, subworker); edgeMapDenseReduce(GA, All,
    // SPMV_F<vertex>(p_curr, p_next, GA.V, rangeLow,
    // rangeHi),output,false,subworker); edgeMap(GA, All,
    // SPMV_F<vertex>(p_curr,p_next,GA.V,rangeLow,rangeHi),output,0,DENSE_FORWARD,
...
void *SPMVThread(void *arg) {
  ...
  */
  ...
  if (tid == 0) All = new vertices(numOfT);
  ...
  for (int i = 0; i < CORES_PER_NODE; i++) {
    SPMV_subworker_arg *arg =
        (SPMV_subworker_arg *)malloc(sizeof(SPMV_subworker_arg));
...
struct SPMV_Hash_F {
  ...
  intT vertPerShard;
  intT n;
  SPMV_Hash_F(intT _n, int _shardNum)
