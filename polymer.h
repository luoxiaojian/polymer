...
struct Subworker_Partitioner {
  ...
  inline intT getEndPos(intT m) {
    return (subTid == numOfSub - 1) ? m : ((subTid + 1) * (m / numOfSub));
  }

  inline void localWait() { local_custom.wait(); }
...
struct Default_Hash_F {
  ...
  intT vertPerShard;
  intT n;
  Default_Hash_F(intT _n, int _shardNum)
...
void partitionByDegree(graph<vertex> GA, int numOfShards, intT *sizeArr,
                       intT sizeOfOneEle, bool useOutDegree = false) {
  const intT n = GA.n;
...
template <class vertex>
void subPartitionByDegree(graph<vertex> GA, int numOfShards, intT *sizeArr,
...
template <class vertex>
void subPartitionByDegree(graph<vertex> GA, int numOfShards, intT *sizeArr,
                          int sizeOfOneEle, intT subStart, intT subEnd,
                          bool useOutDegree = false,
                          bool useFakeDegree = false) {
  ...
  if (useFakeDegree) {
    {
      parallel_for(intT i = subStart; i < subEnd; i++) degrees[i - subStart] =
          GA.V[i].getFakeDegree();
    }
  } else {
    if (useOutDegree) {
      {
...
template <class vertex>
graph<vertex> graphFilter(graph<vertex> &GA, intT rangeLow, intT rangeHi,
                          bool useOutEdge = true) {
  vertex *V = GA.V;
  ...
  intT *offsets = (intT *)numa_alloc_local(sizeof(intT) * GA.n);
  {
    parallel_for(intT i = 0; i < GA.n; i++) {
      ...
      for (intT j = 0; j < d; j++) {
        intT ngh =
            (useOutEdge) ? (V[i].getOutNeighbor(j)) : (V[i].getInNeighbor(j));
        if (rangeLow <= ngh && ngh < rangeHi) counters[i]++;
      }
...
template <class vertex>
graph<vertex> graphFilter2Direction(graph<vertex> &GA, intT rangeLow,
                                    intT rangeHi) {
  vertex *V = GA.V;
  vertex *newVertexSet = (vertex *)numa_alloc_local(sizeof(vertex) * GA.n);
  intT *counters = (intT *)numa_alloc_local(sizeof(intT) * GA.n);
  ...
  intT *inOffsets = (intT *)numa_alloc_local(sizeof(intT) * GA.n);
  {
    parallel_for(intT i = 0; i < GA.n; i++) {
      ...
      for (intT j = 0; j < d; j++) {
        ...
        if (rangeLow <= ngh && ngh < rangeHi) inCounters[i]++;
      }
      newVertexSet[i].setFakeDegree(counters[i]);
...

  {
    parallel_for(intT i = 0; i < GA.n; i++) {
      ...
      if (counter != newVertexSet[i].getFakeInDegree()) {
        printf("oops: %" PRIintT " %" PRIintT "\n", counter,
               newVertexSet[i].getFakeInDegree());
      }
...
void *mapDataArray(int numOfShards, intT *sizeArr, int sizeOfOneEle) {
  ...
  void *toBeReturned = mmap(NULL, numOfPages * PAGESIZE, PROT_READ | PROT_WRITE,
...
struct LocalFrontier {
  ...
  LocalFrontier(bool *_b, intT start, intT end)
      : b(_b),
        startID(start),
        endID(end),
        n(end - start),
  ...
  void toDenseWithMerge(int numOfSub) {
    if (!isDense) {
      ...
      for (int i = 0; i < numOfSub; i++) {
        ...
        {
          parallel_for(intT j = 0; j < size; j++) b[sparsePtr[j] - startID] =
              true;
        }
      }
    }
...
struct vertices {
  ...
  vertices(int _numOfNodes) {
    ...
    nextFrontiers =
        (LocalFrontier **)malloc(numOfNodes * sizeof(LocalFrontier *));
  ...
  */
  ...
  void toDense() {
    ...
    for (int i = 0; i < numOfNodes; i++) {
      ...
      // frontiers[i]->s[frontiers[i]->m - 1], frontiers[i]->n);
      frontiers[i]->toDense();
    }
  }
  ...
  intT getSize(int nodeNum) { return numOfVertexOnNode[nodeNum]; }

  intT getSparseSize(int nodeNum) { return numOfNonZero[nodeNum]; }

  void calculateNumOfNonZero(int nodeNum) {
    numOfNonZero[nodeNum] = 0;
    if (false && frontiers[nodeNum]->isDense) {
      numOfNonZero[nodeNum] =
          sequence::sum(frontiers[nodeNum]->b, numOfVertexOnNode[nodeNum]);
  ...
  intT getOffset(int nodeNum) { return offsets[nodeNum]; }
  ...
  bool *getArr(int nodeNum) { return frontiers[nodeNum]->b; }
  ...
  intT *getSparseArr(int nodeNum) { return frontiers[nodeNum]->s; }

  LocalFrontier *getFrontier(int nodeNum) { return frontiers[nodeNum]; }
  ...
  bool eq(vertices &b) { return false; }
...
enum options { DENSE, DENSE_PARALLEL, DENSE_FORWARD };
...
bool *edgeMapDense(graph<vertex> GA, vertices *frontier, F f,
                   LocalFrontier *next, bool parallel = 0,
...
template <class F, class vertex>
bool *edgeMapDenseForward(graph<vertex> GA, vertices *frontier, F f,
                          LocalFrontier *next, bool part = false,
                          intT start = 0, intT end = 0) {
  intT numVertices = GA.n;
  vertex *G = GA.V;
  ...
  for (intT i = startPos; i < endPos; i++) {
    ...
    if (currBitVector[i - currOffset]) {
      ...
      for (intT j = 0; j < d; j++) {
        ...
        //__builtin_prefetch(f.nextPrefetchAddr(G[i].getOutNeighbor(j+1)), 1,
        // 0);
      }
    }
...
template <class F, class vertex>
bool *edgeMapDenseForwardDynamic(
    graph<vertex> GA, vertices *frontier, F f, LocalFrontier *next,
    Subworker_Partitioner &subworker = dummyPartitioner) {
  intT numVertices = GA.n;
  vertex *G = GA.V;
  ...
  intT endPos = (startPos + DYNAMIC_CHUNK_SIZE > numVertices)
                    ? (numVertices)
                    : (startPos + DYNAMIC_CHUNK_SIZE);
  while (startPos < numVertices) {
    ...
    endPos = (startPos + DYNAMIC_CHUNK_SIZE > numVertices)
                 ? (numVertices)
                 : (startPos + DYNAMIC_CHUNK_SIZE);
  }
  return NULL;
...
template <class F, class vertex>
bool *edgeMapDenseReduce(graph<vertex> GA, vertices *frontier, F f,
                         LocalFrontier *next, bool parallel = 0,
                         Subworker_Partitioner &subworker = dummyPartitioner) {
  intT numVertices = GA.n;
  intT size = next->endID - next->startID;
...
template <class F, class vertex>
bool *edgeMapDenseDynamic(graph<vertex> GA, vertices *frontier, F f,
                          LocalFrontier *next,
                          Subworker_Partitioner &subworker = dummyPartitioner) {
  intT numVertices = GA.n;
  vertex *G = GA.V;
...
template <class F, class vertex>
bool *edgeMapDenseBP(graph<vertex> GA, vertices *frontier, F f,
                     LocalFrontier *next, bool part = false, intT start = 0,
                     intT end = 0) {
  intT numVertices = GA.n;
  vertex *G = GA.V;
  ...
  for (intT i = startPos; i < endPos; i++) {
    ...
    if (currBitVector[i - currOffset]) {
      ...
      for (intT j = 0; j < d; j++) {
        uintT ngh = G[i].getOutNeighbor(j);
        if (/*next->inRange(ngh) &&*/ f.cond(ngh) &&
            f.updateAtomic(i, ngh, j)) {
          next->setBit(ngh, true);
        }
      }
    }
...
template <class F, class vertex>
bool *edgeMapDenseForwardGlobalWrite(graph<vertex> GA, vertices *frontier, F f,
                                     LocalFrontier *nexts[],
  ...
  // subworker.subTid, startPos, endPos);
...
template <class F, class vertex>
void edgeMapSparseAsync(graph<vertex> GA, vertices *frontier, F f,
                        LocalFrontier *next,
                        Subworker_Partitioner &subworker = dummyPartitioner) {
  ...
  *endSignal = 0;
    ...
    */
    do {
      ...
      endPos = MIN(currHead + 1, currTail);
        ...
        !__sync_bool_compare_and_swap((intT *)queueHead, currHead, endPos));
    ...
    if (reallyGotOne > 0) {
      *localSignal = 0;
      ...
      if (currChunk == NULL) {
        printf("oops: %p %p %" PRIintT " %" PRIintT " %" PRIintT "\n",
               currChunk, frontier->asyncQueue[currHead % GA.n], currHead,
               currTail, endPos);
      }
          ...
          if (f.cond(ngh) && f.updateAtomic(idx, ngh)) {
            ...
            if (myChunk->m >= BLOCK_SIZE) {
              ...
              */
              ...
              while (!__sync_bool_compare_and_swap((intT *)queueTail, insertPos,
                                                   insertPos + 1)) {
      ...
      */
      ...
      // subworker.subTid, currHead, currTail);
      if (*endSignal == 1) {
  ...
  // subworker.subTid, accumSize);
}

template <class F, class vertex>
void edgeMapSparseAsyncPipe(
    graph<vertex> GA, vertices *frontier, F f, LocalFrontier *next,
    Subworker_Partitioner &subworker = dummyPartitioner) {
  const int BLOCK_SIZE = 64;
  vertex *V = GA.V;
  ...
  frontier->frontiers[tid]->tmp =
      (intT *)malloc(sizeof(intT) * frontier->getSize(tid));
  pthread_barrier_wait(subworker.local_barr);

  AsyncChunk **localQueue = frontier->frontiers[tid]->localQueue;
  AsyncChunk **nextQueue =
      frontier->frontiers[(tid + 1) % frontier->numOfNodes]->localQueue;

  volatile intT *localHead = &(frontier->frontiers[tid]->head);
  volatile intT *localTail = &(frontier->frontiers[tid]->tail);
  volatile intT *nextTail =
      &(frontier->frontiers[(tid + 1) % frontier->numOfNodes]->tail);
  volatile intT *insertTail =
      &(frontier->frontiers[(tid + 1) % frontier->numOfNodes]->insertTail);
  volatile intT *localSignal = &(frontier->frontiers[tid]->emptySignal);
  volatile intT *endSignal = &(frontier->asyncEndSignal);
  ...
  while (!shouldFinish) {
    ...
    do {
      ...
      endPos = MIN(currHead + 1, currTail);
        ...
        !__sync_bool_compare_and_swap((intT *)localHead, currHead, endPos));
    ...
    if (reallyGotOne > 0) {
      *localSignal = 0;
      ...
      if (chunkSize > 0) {
              ...
              if (counter == 0) {
                ...
                if (myChunk->m >= BLOCK_SIZE) {
                  ...
                  while (!__sync_bool_compare_and_swap(
                      (intT *)nextTail, insertPos, insertPos + 1)) {
                    if (*nextTail > insertPos) {
                      break;
                      printf("pending on insert %" PRIintT " %" PRIintT
                             " %" PRIintT "\n",
        ...
        if (chunkSize < 0) {
          // end game message.
          if (currChunk->accessCounter >= 2 * frontier->numOfNodes &&
              subworker.isMaster()) {
            *endSignal = 1;
            printf("master out\n");
            shouldFinish = true;
          ...
          if (*localHead >= *localTail && myChunk->m <= 0) {
            ...
            while (!__sync_bool_compare_and_swap((intT *)nextTail, insertPos,
                                                 insertPos + 1)) {
              if (*nextTail > insertPos) {
                break;
                printf("pending on insert %" PRIintT " %" PRIintT " %" PRIintT
      ...
      if (myChunk->m > 0) {
        ...
        // (tid + 1) % frontier->numOfNodes, insertPos);
        myChunk = newChunk(BLOCK_SIZE);
      }
...
template <class F, class vertex>
void edgeMapSparseV5(graph<vertex> GA, vertices *frontier, F f,
                     LocalFrontier *next,
                     Subworker_Partitioner &subworker = dummyPartitioner) {
  vertex *V = GA.V;
  ...
  while (startPos < endPos) {  // only master get in
    ...
    intT lengthOfCurr =
        frontier->getSparseSize(currNodeNum) - (startPos - offset);
    for (intT i = startPos; i < endPos; i++) {
      if (lengthOfCurr <= 0) {
...
template <class F, class vertex>
void edgeMapSparseV4(graph<vertex> GA, vertices *frontier, F f,
                     LocalFrontier *next, bool firstTime = false,
                     Subworker_Partitioner &subworker = dummyPartitioner) {
...
template <class F, class vertex>
void edgeMapSparseV3(graph<vertex> GA, vertices *frontier, F f,
                     LocalFrontier *next, bool part = false,
                     Subworker_Partitioner &subworker = dummyPartitioner) {
  vertex *V = GA.V;
  if (part) {
    ...
    if (startPos < endPos) {
      ...
      for (intT i = startPos; i < endPos; i++) {
        ...
        for (intT j = 0; j < d; j++) {
          ...
          if (f.cond(ngh) && f.updateAtomic(idx, ngh)) {
            // add to active list
            // printf("out edge # %" PRIintT ": %" PRIintT " -> %" PRIintT " of
...
template <class F, class vertex>
void edgeMapSparseV2(graph<vertex> GA, vertices *frontier, F f,
                     LocalFrontier *next, bool part = false,
                     Subworker_Partitioner &subworker = dummyPartitioner) {
  vertex *V = GA.V;
  if (part) {
    ...
    /*
    if (subworker.isMaster()) {
        double timeStart = ((double)start1.tv_sec) + ((double)start1.tv_usec) /
    1000000.0; double timeEnd = ((double)end1.tv_sec) + ((double)end1.tv_usec) /
    1000000.0; double mapTime = timeEnd - timeStart; printf("#1:after map time:
    %lf\n", mapTime);

        timeStart = ((double)start2.tv_sec) + ((double)start2.tv_usec) /
    1000000.0; timeEnd = ((double)end2.tv_sec) + ((double)end2.tv_usec) /
    1000000.0; mapTime = timeEnd - timeStart; printf("#2:after map time: %lf\n",
    mapTime);

        timeStart = ((double)start3.tv_sec) + ((double)start3.tv_usec) /
    1000000.0; timeEnd = ((double)end3.tv_sec) + ((double)end3.tv_usec) /
    1000000.0; mapTime = timeEnd - timeStart; printf("#3:after map time: %lf\n",
    mapTime);

        timeStart = ((double)startT.tv_sec) + ((double)startT.tv_usec) /
    1000000.0; timeEnd = ((double)endT.tv_sec) + ((double)endT.tv_usec) /
    1000000.0; mapTime = timeEnd - timeStart; printf("Total:after map time:
    %lf\n", mapTime);
    }
    */
  }
...
void edgeMapSparse(graph<vertex> GA, vertices *frontier, F f, Vert_F vf,
                   LocalFrontier *next,
                   Subworker_Partitioner &subworker = dummyPartitioner) {
  vertex *V = GA.V;
  intT sparseIter = 0;
  ...
  while (true) {
    ...
    uintT outEdgeCount =
        sequence::plusScan(offsets, (uintT *)degrees, (uintT)totM);
    intT newM = 0;
    ...
    if (newM <= 0) {
      ...
      totM = newM;
      {
        parallel_for(intT i = 0; i < totM; i++) { vf(sparseQueue[i]); }
      }
    }
  }
}
...
void clearLocalFrontier(LocalFrontier *next, int nodeNum, int subNum,
                        int totalSub);

// decides on sparse or dense base on number of nonzeros in the active vertices
template <class F, class vertex>
void edgeMap(graph<vertex> GA, vertices *V, F f, LocalFrontier *next,
  ...
  if (m >= threshold) {
    ...
    if (subworker.isSubMaster()) {
        if (next->sparseChunks == NULL) {
            next->sparseChunks = (intT **)malloc(sizeof(intT *) *
    subworker.numOfSub);
        }
        if (next->chunkSizes == NULL) {
            next->chunkSizes = (intT *)malloc(sizeof(intT) *
    subworker.numOfSub);
        }
    }
    */
...
//*****VERTEX FUNCTIONS*****
template <class vertex>
void vertexCounter(graph<vertex> GA, LocalFrontier *frontier, int nodeNum,
                   int subNum, int totalSub) {
  if (!frontier->isDense) return;
...
void vertexFilter(vertices *V, F filter, int nodeNum, bool *result) {
  ...
  for (intT i = 0; i < size; i++) {
    result[i] = false;
    if (b[i]) result[i] = filter(i + offset);
  }
}

template <class F>
void vertexFilter(vertices *V, F filter, int nodeNum, int subNum, int totalSub,
                  LocalFrontier *result) {
  intT size = V->getSize(nodeNum);
  intT offset = V->getOffset(nodeNum);
  bool *b = V->getArr(nodeNum);
  intT subSize = size / totalSub;
