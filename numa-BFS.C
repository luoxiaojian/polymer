...
struct BFS_F {
  ...
  inline void vertUpdate(intT v) { return; }
  // cond function checks if vertex has been visited yet
  inline bool cond(intT d) { return (Parents[d] == -1); }
};

struct BFS_Vert_F {
  inline bool operator()(intT i) { return 1; }
};
...
bool *edgeMapDenseNoRep(graph<vertex> GA, vertices *frontier, F f,
...
template <class F, class vertex>
void edgeMapNoRep(graph<vertex> GA, vertices *V, F f, LocalFrontier *next,
...
void *BFSSubWorker(void *arg) {
  ...
  while (!Frontier->isEmpty() ||
         currIter == 0) {  // loop until frontier is empty
    currIter++;
    if (tid + subTid == 0) {
    ...
    edgeMapNoRep(GA, Frontier, BFS_F(parents), output, 0, DENSE_PARALLEL, false,
                 true, subworker);
    subworker.localWait();
...
void *BFSWorker(void *arg) {
  ...
  */
  ...
  for (int i = 0; i < CORES_PER_NODE; i++) {
    BFS_subworker_arg *arg =
        (BFS_subworker_arg *)malloc(sizeof(BFS_subworker_arg));
  ...
  double timeStart =
      ((double)timer_start.tv_sec) + ((double)timer_start.tv_usec) / 1000000.0;
  double timeEnd =
      ((double)timer_end.tv_sec) + ((double)timer_end.tv_usec) / 1000000.0;

  double mapTime = timeEnd - timeStart;
  if (tid == 0) printf("self counted time: %lf\n", mapTime);
...
int parallel_main(int argc, char *argv[]) {
  ...
  if (symmetric) {
    graph<symmetricVertex> G = readGraph<symmetricVertex>(
        iFile, symmetric, binary);  // symmetric graph
    BFS((intT)start, G);
    // G.del();
  } else {
    graph<asymmetricVertex> G = readGraph<asymmetricVertex>(
        iFile, symmetric, binary);  // asymmetric graph
    BFS((intT)start, G);
    // G.del();
  }
}
