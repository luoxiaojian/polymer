...
void *BFSSubWorker(void *arg) {
  ...
  while (!Frontier->isEmpty() ||
    ...
    if (subTid == 0) {
      //{parallel_for(intT i=output->startID;i<output->endID;i++)
      //output->setBit(i, false);}
    }
    // pthread_barrier_wait(global_barr);
    // apply edgemap
    gettimeofday(&startT, &tz);
    // edgeMap(GA, Frontier, BFS_F(parents), output, GA.n/20, DENSE_FORWARD,
    // false, true, subworker); vertexCounter(GA, output, tid, subTid,
    // CORES_PER_NODE);
...
void *BFSWorker(void *arg) {
  ...
  */
  ...
  current->head = 0;
  LocalFrontier *toInsert =
      Frontier->frontiers[(tid + 1) % Frontier->numOfNodes];
  if (my_arg->start >= rangeLow && my_arg->start < rangeHi) {
