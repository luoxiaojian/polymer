...
void partitionByDegree(wghGraph<vertex> GA, int numOfShards, intT *sizeArr,
...
template <class vertex>
void subPartitionByDegree(wghGraph<vertex> GA, int numOfShards, intT *sizeArr,
                          int sizeOfOneEle, bool useOutDegree = false,
...
template <class vertex>
wghGraph<vertex> graphFilter(wghGraph<vertex> &GA, intT rangeLow, intT rangeHi,
  ...
  intE *edges =
      (intE *)malloc((long long)sizeof(intE) * totalSize * (long long)2);
    ...
    parallel_for(intT i = 0; i < GA.n; i++) {
      ...
      for (intT j = 0; j < d; j++) {
        intT ngh =
            (useOutEdge) ? (V[i].getOutNeighbor(j)) : (V[i].getInNeighbor(j));
        intT wgh =
            (useOutEdge) ? (V[i].getOutWeight(j)) : (V[i].getInWeight(j));
...
template <class vertex>
wghGraph<vertex> graphFilter2Direction(wghGraph<vertex> &GA, intT rangeLow,
                                       intT rangeHi, bool useOutEdge = true) {
  vertex *V = GA.V;
  vertex *newVertexSet = (vertex *)numa_alloc_local(sizeof(vertex) * GA.n);
  intT *counters = (intT *)numa_alloc_local(sizeof(intT) * GA.n);
  intT *offsets = (intT *)numa_alloc_local(sizeof(intT) * GA.n);
  ...
  intE *edges =
      (intE *)malloc((long long)sizeof(intE) * totalSize * (long long)2);
  intE *inEdges =
      (intE *)malloc((long long)sizeof(intE) * totalInSize * (long long)2);
...
bool *edgeMapDense(wghGraph<vertex> GA, vertices *frontier, F f,
  ...
  for (intT i = startPos; i < endPos; i++) {
    ...
    if (f.cond(i)) {
      ...
      for (intT j = 0; j < d; j++) {
        ...
        if (localBitVec[ngh - localOffset] &&
            f.updateAtomic(ngh, i, G[i].getInWeight(j))) {
...
template <class F, class vertex>
bool *edgeMapDenseForward(wghGraph<vertex> GA, vertices *frontier, F f,
    ...
    if (currBitVector[i - currOffset]) {
      ...
      for (intT j = 0; j < d; j++) {
        ...
        if (/*next->inRange(ngh) &&*/ f.cond(ngh) &&
            f.updateAtomic(i, ngh, G[i].getOutWeight(j))) {
          /*
          if (!next->getBit(ngh)) {
              m++;
...
template <class F, class vertex>
bool *edgeMapDenseReduce(wghGraph<vertex> GA, vertices *frontier, F f,
                         LocalFrontier *next, bool parallel = 0,
                         Subworker_Partitioner &subworker = dummyPartitioner) {
  intT numVertices = GA.n;
  intT size = next->endID - next->startID;
  ...
  for (intT i = startPos; i < endPos; i++) {
    ...
    if (true || f.cond(i)) {
      ...
      for (intT j = 0; j < d; j++) {
        intT ngh = G[i].getInNeighbor(j);
        if (/*localBitVec[ngh - localOffset] && */ f.reduceFunc(
            (void *)data, ngh, G[i].getInWeight(j))) {
          currBitVector[i - currOffset] = true;
          // shouldActive = true;
        }
        // if(!f.cond(i)) break;
...
bool *edgeMapDenseForwardDynamic(
    wghGraph<vertex> GA, vertices *frontier, F f, LocalFrontier *next,
    Subworker_Partitioner &subworker = dummyPartitioner) {
  intT numVertices = GA.n;
  vertex *G = GA.V;
...
template <class F, class vertex>
bool *edgeMapDenseForwardGlobalWrite(wghGraph<vertex> GA, vertices *frontier,
                                     F f, LocalFrontier *nexts[],
  ...
  for (intT i = startPos; i < endPos; i++) {
    ...
    if (f.cond(i)) {
      ...
      for (intT j = 0; j < d; j++) {
        ...
        if (currBitVector[ngh - currOffset] &&
            f.updateAtomic(ngh, i, G[i].getInWeight(j))) {
...
template <class F, class vertex>
void edgeMapSparseAsync(wghGraph<vertex> GA, vertices *frontier, F f,
                        LocalFrontier *next,
                        Subworker_Partitioner &subworker = dummyPartitioner) {
...
template <class F, class vertex>
void edgeMapSparseV3(wghGraph<vertex> GA, vertices *frontier, F f,
                     LocalFrontier *next, bool part = false,
                     Subworker_Partitioner &subworker = dummyPartitioner) {
  vertex *V = GA.V;
  if (part) {
...
template <class F, class vertex>
void edgeMap(wghGraph<vertex> GA, vertices *V, F f, LocalFrontier *next,
...
//*****VERTEX FUNCTIONS*****
template <class vertex>
void vertexCounter(wghGraph<vertex> GA, LocalFrontier *frontier, int nodeNum,
