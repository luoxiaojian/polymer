...
struct BP_F {
  ...
  intT *offsets;
  intT rangeLow;
  BP_F(EdgeWeight *_edgeW, EdgeData *_edgeD_curr, EdgeData *_edgeD_next,
       VertexInfo *_vertI, VertexData *_vertD_curr, VertexData *_vertD_next,
       intT *_offsets, intT _rangeLow = 0)
      : edgeW(_edgeW),
        edgeD_curr(_edgeD_curr),
        edgeD_next(_edgeD_next),
        vertI(_vertI),
        vertD_curr(_vertD_curr),
        vertD_next(_vertD_next),
        offsets(_offsets),
        rangeLow(_rangeLow) {}
  inline bool update(intT s, intT d, intT edgeIdx) {
    ...
    for (int i = 0; i < NSTATES; i++) {
      ...
      for (int j = 0; j < NSTATES; j++) {
        edgeD_next[dstIdx].belief[i] += vertI[d].potential[j] *
                                        edgeW[dstIdx].potential[i][j] *
                                        vertD_curr[d].product[j];
      }
      vertD_next[d].product[i] =
          vertD_next[d].product[i] * edgeD_next[dstIdx].belief[i];
    }
    return 1;
  }
...
struct BP_Vertex_Reset {
  VertexData *vertD;
  BP_Vertex_Reset(VertexData *_vertD) : vertD(_vertD) {}
...
bool *edgeMapDenseBPNoRep(graph<vertex> GA, vertices *frontier, F f,
...
void *BeliefPropagationSubWorker(void *arg) {
  ...
  while (1) {
    ...
    if (subTid == 0) {
      {
        parallel_for(long i = output->startID; i < output->endID; i++)
    ...
    vertexMap(Frontier, BP_Vertex_Reset(vertD_next), tid, subTid,
              CORES_PER_NODE);
    output->m = 1;
    pthread_barrier_wait(&global_barr);

    // edgeMapDenseBP(GA, Frontier, BP_F<vertex>(edgeW, edgeD_curr, edgeD_next,
    // vertI, vertD_curr, vertD_next, localOffsets),output,true,start,end);
    edgeMapDenseBPNoRep(
        GA, Frontier,
        BP_F<vertex>(edgeW, edgeD_curr, edgeD_next, vertI, vertD_curr,
                     vertD_next, localOffsets2, rangeLow),
        output, true, subworker);
    pthread_barrier_wait(&global_barr);
...
void *BeliefPropagationThread(void *arg) {
  ...
  intT numLocalEdge =
      localOffsets[localGraph.n - 1] + fakeDegrees[localGraph.n - 1];
  ...
  numLocalEdge = localOffsets2[rangeHi - rangeLow - 1] +
                 localDegrees[rangeHi - rangeLow - 1];
  printf("numLocalEdge of %d: %" PRIintT "\n", tid, numLocalEdge);

  EdgeWeight *edgeW =
      (EdgeWeight *)numa_alloc_local(sizeof(EdgeWeight) * numLocalEdge);

  EdgeData *edgeD_curr =
      (EdgeData *)numa_alloc_local(sizeof(EdgeData) * numLocalEdge);
  EdgeData *edgeD_next =
      (EdgeData *)numa_alloc_local(sizeof(EdgeData) * numLocalEdge);
  /*
  ...
  */
  ...
  subPartitionByDegree(localGraph, CORES_PER_NODE, sizeOfShards,
                       sizeof(VertexData), true, true);
  ...
  */
  ...
  for (int i = 0; i < CORES_PER_NODE; i++) {
    BP_subworker_arg *arg =
        (BP_subworker_arg *)malloc(sizeof(BP_subworker_arg));
    ...
    startPos = arg->endPos;
    pthread_create(&subTids[i], NULL, BeliefPropagationSubWorker<vertex>,
                   (void *)arg);
  }
...
struct BP_Hash_F {
  ...
  intT vertPerShard;
  intT n;
  BP_Hash_F(intT _n, int _shardNum)
...
void BeliefPropagation(graph<vertex> &GA, int maxIter) {
  ...
  */
  ...
  VertexData *vertD_curr =
      (VertexData *)mapDataArray(numOfNode, sizeArr, sizeof(VertexData));
  VertexData *vertD_next =
      (VertexData *)mapDataArray(numOfNode, sizeArr, sizeof(VertexData));
  ...
  for (int i = 0; i < numOfNode; i++) {
    ...
    prev = prev + sizeArr[i];
    pthread_create(&tids[i], NULL, BeliefPropagationThread<vertex>,
                   (void *)arg);
  }
  shouldStart = 1;
  pthread_barrier_wait(&timerBarr);
