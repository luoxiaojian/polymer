...
struct PR_F {
  ...
  intT rangeLow;
  intT rangeHi;
  PR_F(double *_p_curr, double *_p_next, vertex *_V, intT _rangeLow,
       intT _rangeHi)
      : p_curr(_p_curr),
        p_next(_p_next),
        V(_V),
  ...
  inline void *nextPrefetchAddr(intT index) { return &p_curr[index]; }
  inline bool update(intT s,
                     intT d) {  // update function applies PageRank equation
    p_next[d] += p_curr[s] / V[s].getOutDegree();
    return 1;
  }
  inline double getCurrVal(intT i) { return p_curr[i]; }
  ...
  inline bool updateAtomic(intT s, intT d) {  // atomic Update
    ...
    std::setprecision(9) << p_curr[s]/V[s].getOutDegree() << " -- " << p_next[d]
    << "\n";
    }
    */
    return 1;
  ...
  inline void initFunc(void *dataPtr, intT d) { *(double *)dataPtr = 0.0; }

  inline bool reduceFunc(void *dataPtr, intT s, bool print_info = false) {
    *(double *)dataPtr += p_curr[s] / (double)V[s].getOutDegree();
    if (print_info) {
      // cout << "reduce: " << s << " " << std::scientific <<
      // std::setprecision(9) << p_curr[s] / (double)V[s].getOutDegree() << " "
      // << p_curr[s] << " " << V[s].getOutDegree() << *(double *)dataPtr <<
      // "\n";
    }
    return true;
  }
  ...
  inline bool combineFunc(void *dataPtr, intT d) {
    ...
    /*
    if (d == 77) {
        cout << "combine result: " << std::scientific << std::setprecision(9) <<
    val << " " << p_next[d] << "\n";
    }
    */
    return true;
...
bool *edgeMapDenseForwardOTHER(graph<vertex> GA, vertices *frontier, F f,
...
void *PageRankSubWorker(void *arg) {
  ...
  while (1) {
    ...
    currIter++;
    if (subTid == 0) Frontier->calculateNumOfNonZero(tid);
    if (subTid == 0) {
    ...
    // edgeMap(GA, Frontier,
    // PR_F<vertex>(p_curr,p_next,GA.V,rangeLow,rangeHi),output,0,DENSE_FORWARD,
    // false, true, subworker);
    clearLocalFrontier(output, subworker.tid, subworker.subTid,
                       subworker.numOfSub);
    output->sparseCounter = 0;
    subworker.globalWait();
    // edgeMapDenseReduce(GA, Frontier,
    // PR_F<vertex>(p_curr,p_next,GA.V,rangeLow,rangeHi),output,false,subworker);

    subworker.localWait();
    struct timeval startT, endT;
    struct timezone tz = {0, 0};
    gettimeofday(&startT, &tz);
    // edgeMapDenseForward(GA, Frontier,
    // PR_F<vertex>(p_curr,p_next,GA.V,rangeLow,rangeHi),output, true,
    // subworker.dense_start, subworker.dense_end);
    edgeMapDenseForwardOTHER(
        GA, Frontier, PR_F<vertex>(p_curr, p_next, GA.V, rangeLow, rangeHi),
        output, true, subworker.dense_start, subworker.dense_end);
    // edgeMapDenseForwardDynamic(GA, Frontier,
    // PR_F<vertex>(p_curr,p_next,GA.V,rangeLow,rangeHi),output, subworker);
    subworker.localWait();
    ...
    if (subworker.isSubMaster()) {
      ...
      // printf("time of %d: %lf\n", subworker.tid * CORES_PER_NODE +
      // subworker.subTid, duration);
    }
    ...
    vertexMap(Frontier, PR_Vertex_F(p_curr, p_next, damping, n), tid, subTid,
              CORES_PER_NODE);
...
void *PageRankThread(void *arg) {
  ...
  */
  ...
  if (tid == 0) Frontier->calculateOffsets();
  ...
  while(1){
      ...
      //edgeMap(GA, Frontier,
  PR_F<vertex>(p_curr,p_next,GA.V,rangeLow,rangeHi),output,GA.m/20,DENSE_FORWARD);
      pthread_barrier_wait(&localBarr);
...
int parallel_main(int argc, char *argv[]) {
  ...
  if (argc > 4)
    if ((string)argv[4] == (string) "-result") needResult = true;
  if (argc > 5)
    if ((string)argv[5] == (string) "-s") symmetric = true;
  if (argc > 6)
    if ((string)argv[6] == (string) "-b") binary = true;
  numa_set_interleave_mask(numa_all_nodes_ptr);
  startTime();
  if (symmetric) {
    graph<symmetricVertex> G = loadGraphFromBin<symmetricVertex>(iFile);
    PageRank(G, maxIter);
    // G.del();
  } else {
    graph<asymmetricVertex> G = loadGraphFromBin<asymmetricVertex>(iFile);
    PageRank(G, maxIter);
    // G.del();
  }
  return 0;
